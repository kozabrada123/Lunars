...
pub fn update_player(player: &mut db::Player, matches: Vec<db::Match>) {
    ...
    player.volatility = new_vol(player.clone(), matches, v);
    ...
    for i in 0..matches.len() {
        tempsum += g(player.clone(), matches[i].deviation_b)
...
fn e(player: db::Player, rank_b: u16, deviation_b: u16) -> f64 {
    return 1.0 / (1.0 + (-1.0 * g(player, deviation_b) * (player.rank - rank_b) as f64));
}
